...
class GraphUtil:
    ...
    #                             expected_results = (step.bug.description.expected_results.text,
    #                                                 GraphUtil.get_concepts_name_alias_list(
    #                                                     step.bug.description.expected_results.concepts))
    #                         if step.bug.description.actual_results:
    #                             actual_results = (step.bug.description.actual_results.text,
    #                                               GraphUtil.get_concepts_name_alias_list(
    #                                                   step.bug.description.actual_results.concepts))
                ...
                for step in cluster:
                    if step.next_step is None:
                        if step.bug.description.expected_results or step.bug.description.actual_results:
                            ...
                            actual_results = None
                            if step.bug.description.expected_results:
                                expected_results = step.bug.description.expected_results
                            if step.bug.description.actual_results:
                                actual_results = step.bug.description.actual_results
                            bugid_stepid_expected_actual_result_list.append((step.bug.id, step.id,
                                                                             expected_results,
                                                                             actual_results))
                GraphUtil.INDEX_CLUSTER_EXPECTED_ACTUAL_RESULT_DICT[cluster_index] = \
    ...
    @staticmethod
    def get_step_cluster_for_step(step_text, bug_pair):
        for step in bug_pair[0].description.steps_to_reproduce:
            if step.text == step_text:
                return step.cluster_index
        for step in bug_pair[1].description.steps_to_reproduce:
            if step.text == step_text:
                return step.cluster_index
        return None

    @staticmethod
    def get_step_dict_for_step_ans(step_ans):
        step_cluster = step_ans[Placeholder.STEP_CLUSTER]
        expected_actual_result_list = GraphUtil.INDEX_CLUSTER_EXPECTED_ACTUAL_RESULT_DICT[step_cluster]
        ...
        for expected_actual_result in expected_actual_result_list:
            expected_actual_result_dict = {
                ...
                'ExpectedResults': GraphUtil.convert_list_into_str(expected_actual_result[2]),
                'ExpectedResultsElements': [],
                'ActualResults': GraphUtil.convert_list_into_str(expected_actual_result[3]),
                'ActualResultsElements': []
            }
            expected_actual_result_dict_list.append(expected_actual_result_dict)

        step_concepts_list = []
        step_dict = {
            'ClusterIndex': step_cluster,
            'StepText': step_ans[Placeholder.STEP],
    ...
    @staticmethod
    def convert_s2r_ans_into_step_dicts(s2r_ans):
        step_dicts = []
        for step_ans in s2r_ans:
            step_dict = GraphUtil.get_step_dict_for_step_ans(step_ans)
            step_dicts.append(step_dict)
        return step_dicts

    @staticmethod
    def convert_ans_into_test_report_dicts(ans):
        test_report_dicts = list()
        chains = None
        if Placeholder.CHAINS_OF_THOUGHT in ans.keys():
            chains = ans[Placeholder.CHAINS_OF_THOUGHT]
        scenarios = ans[Placeholder.SCENARIOS]
        for scenario in scenarios:
            test_report_dict = {
                # 'id': None,
                # 'Summary': None,
                'Preconditions': GraphUtil.convert_list_into_str(scenario[Placeholder.PRECONDITIONS]),
                'PreconditionsElements': [],
                'StepsToReproduce': GraphUtil.convert_s2r_ans_into_step_dicts(scenario[Placeholder.STEPS_TO_REPRODUCE]),
                'ExpectedResults': GraphUtil.convert_list_into_str(scenario[Placeholder.EXPECTED_RESULTS]),
                'ExpectedResultsElements': [],
                'ActualResults': GraphUtil.convert_list_into_str(scenario[Placeholder.ACTUAL_RESULTS]),
                'ActualResultsElements': [],
                # 'Notes': GraphUtil.convert_list_into_str(scenario[Placeholder.NOTES]),
                # 'NotesElements': [],
                'SeeAlso': chains,
                # 'Attachments':
            }
            test_report_dicts.append(test_report_dict)
        return test_report_dicts

    @staticmethod
    def get_test_reports_from_two_bugs_by_llm(bug1, bug2, with_instances=None):
        """
        generate test reports from two bugs

        @param with_instances: bugs
        @type with_instances: bugs
        @param bug1:
        @type bug1: Bug
        @param bug2:
        @type bug2: Bug
        ...
        """
        test_report_dict_list = list()
        answer, _ = ScenarioLinker.link_scenario((bug1, bug2), with_instances)
        answer = json.loads(answer)
        # print(type(answer))
        if answer:
            test_report_dict_list = GraphUtil.convert_ans_into_test_report_dicts(answer)
        answer, _ = ScenarioCombiner.combine_scenario((bug1, bug2), with_instances)
        answer = json.loads(answer)
        if answer:
            # for answer in answers[Placeholder.SCENARIOS]:
            test_report_dict_list.extend(GraphUtil.convert_ans_into_test_report_dicts(answer))

        return test_report_dict_list

    @staticmethod
    def get_test_reports_from_two_bug_ids_by_file(bug_id_1, bug_id_2):
        """
        generate test reports from file
        @param bug_id_1:
        @type bug_id_1: int
        @param bug_id_2:
        @type bug_id_2: int
        ...
        @rtype:
        """
        input_bug_ids = {bug_id_1, bug_id_2}
        print(input_bug_ids)
        test_report_dict_list = []
        answers = FileUtil.load_json(PathUtil.get_scenarios_filepath())
        for answer in answers:
            bug_ids = {answer['bug_id_pair'][0], answer['bug_id_pair'][1]}

            if input_bug_ids == bug_ids:
                # print(f"{bug_ids}***********************")
                test_report_dict_list.extend(GraphUtil.convert_ans_into_test_report_dicts(answer['answer']))
    ...
    def get_test_reports_by_merging_two_bugs(bug1, bug2):
        ...
        if bug1.description.prerequisites:
            ...
            test_report_dict['PreconditionsElements'] = GraphUtil.get_concepts_name_alias_list(
                bug1.description.prerequisites.concepts)
        bug1_step_list = list()
        ...
        if bug2.description.expected_results:
            ...
            test_report_dict['ExpectedResultsElements'] = GraphUtil.get_concepts_name_alias_list(
                bug2.description.expected_results.concepts)
        if bug2.description.actual_results:
            test_report_dict['ActualResults'] = bug2.description.actual_results.text
            test_report_dict['ActualResultsElements'] = GraphUtil.get_concepts_name_alias_list(
                bug2.description.actual_results.concepts)
    ...
    def get_test_reports_by_two_steps(step1, step2):
        ...
        if step1.bug.description.prerequisites:
            ...
            test_report_dict['PreconditionsElements'] = GraphUtil.get_concepts_name_alias_list(
                step1.bug.description.prerequisites.concepts)
        ...
        if step2.bug.description.expected_results:
            ...
            test_report_dict['ExpectedResultsElements'] = GraphUtil.get_concepts_name_alias_list(
                step2.bug.description.expected_results.concepts)
        if step2.bug.description.actual_results:
            test_report_dict['ActualResults'] = step2.bug.description.actual_results.text
            test_report_dict['ActualResultsElements'] = GraphUtil.get_concepts_name_alias_list(
                step2.bug.description.actual_results.concepts)
        if step2.bug.description.notes:
            test_report_dict['Notes'] = step2.bug.description.notes.text
            test_report_dict['NotesElements'] = GraphUtil.get_concepts_name_alias_list(
                step2.bug.description.notes.concepts)
        return test_report_dict
    ...
    @staticmethod
    def convert_list_into_str(instance_list):
        instance_str = ""
        if instance_list:
    ...
    def get_bug_dict(bug):
        ...
        if bug.description.prerequisites:
            bug_dict['Preconditions'] = GraphUtil.convert_list_into_str(bug.description.prerequisites)
            bug_dict['PreconditionsElements'] = []
            # bug_dict['PreconditionsElements'] = GraphUtil.get_concepts_name_alias_list(
            #     bug.description.prerequisites.concepts)
        if bug.description.steps_to_reproduce:
            steps = list()
        ...
        if bug.description.expected_results:
            bug_dict['ExpectedResults'] = GraphUtil.convert_list_into_str(bug.description.expected_results)
            # bug_dict['ExpectedResultsElements'] = GraphUtil.get_concepts_name_alias_list(
            #     bug.description.expected_results.concepts)
            bug_dict['ExpectedResultsElements'] = []
        if bug.description.actual_results:
            bug_dict['ActualResults'] = GraphUtil.convert_list_into_str(bug.description.actual_results)
            bug_dict['ActualResultsElements'] = []
            # bug_dict['ActualResultsElements'] = GraphUtil.get_concepts_name_alias_list(
            #     bug.description.actual_results.concepts)
        if bug.description.notes:
            bug_dict['Notes'] = GraphUtil.convert_list_into_str(bug.description.notes)
            bug_dict['NotesElements'] = []
